...
const u16 led_speed_array[MAX_SPEED_RANK] = {
    ...
    300, 350, 400, 450,MIN_SLOW_SPEED
    };//0-500
...
void app_set_bright(u8 tp_b)
{
    ...
    {
        if(tp_b < MIN_BRIGHT_VALUE)  tp_b = MIN_BRIGHT_VALUE;
...
u16 get_max_sp(void)
{
    ...
    if(s<MAX_FAST_SPEED) s= MAX_FAST_SPEED;
    return s; //
...
void ls_add_bright(void)
{
    ...
    if(fc_effect.Now_state == IS_STATIC)
    {
        if(fc_effect.ls_b < (MAX_BRIGHT_RANK-1)) fc_effect.ls_b++;
...
void ls_sub_bright(void)
{
    if(fc_effect.Now_state == IS_STATIC)
    {
        if(fc_effect.ls_b > 0) fc_effect.ls_b--;
...
void ls_add_speed(void)
{
    if(fc_effect.Now_state == IS_light_scene)
    {
        ...
        if(fc_effect.ls_speed > 0)fc_effect.ls_speed--;
...
void ls_sub_speed(void)
{
    ...
    if(fc_effect.Now_state == IS_light_scene)
    {
        ...
        if(fc_effect.ls_speed < (MAX_SPEED_RANK-1)) fc_effect.ls_speed++;
        fc_effect.app_speed =100-(fc_effect.ls_speed)*10;
        fc_effect.dream_scene.speed = led_speed_array[fc_effect.ls_speed];
        fb_speed();
        set_fc_effect();
...
void ls_add_sensitive(void)
{
    ...
    {
        if( fc_effect.music.s < (100 - sen_gap)) fc_effect.music.s += sen_gap;
    }
...
void ls_sub_sensitive(void)
{
    ...
    if(fc_effect.Now_state == IS_light_music)
    {
        if( fc_effect.music.s > sen_gap ) fc_effect.music.s -= sen_gap;
    }
    printf(" fc_effect.music.s= %d",  fc_effect.music.s);
...
{
    if(fc_effect.star_on_off != DEVICE_ON) return;
...
void ls_set_motor_speed(void)
{
    ...
    if(fc_effect.star_speed_index >= 7) fc_effect.star_speed_index  = 0;
